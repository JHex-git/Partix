...
void Renderer::Render()
{
    ...
    particles[0].lifetime = 100000000.0f;
    glGenBuffers(2, ssbo);
    for (int i = 0; i < 2; ++i)
    {
    ...
    // glBindBufferBase(GL_UNIFORM_BUFFER, 0, emitter_ubo);
    ...
    // glBindBufferBase(GL_UNIFORM_BUFFER, 1, frame_ubo);
    ...
    // Emitter emitter;
    // emitter.emitCountPerFrame = 100;
    // emitter.position = glm::vec3(0, 0, 0);
    // emitter.direction = glm::vec3(0, 0, 0);
